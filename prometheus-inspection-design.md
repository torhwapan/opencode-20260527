...
二，智能体功能
1，自动巡检prometheus指标；2，根据用户提问，巡检指定proemtheus服务上的某些指标
...
1,prometheus指标查询工具
2,指标分析工具
3,待巡检指标工具。 一个配置工具，输入prometheus的ip，可以查询用户配置的这个prometheus下的待巡检指标。
----------------------------
指标名称       计算方式 
cpu使用率      cpu使用量/cpu最大值
----------------------------

六，skill
1,prometheus PromQL生成skill，列举一些常见指标的PromQL。
2,
3,报告生成skil。 列举一个通用的报告模板

整个工作流
流程1，调用'待巡检指标工具', 返回需要巡检的指标
流程2，根据流程1返回的指标，调用ai大模型，配合skill1,生成对应的PromQL
流程3，根据PromQL,调用工具1查询promethues近1天的数据量
流程4，调用指标分析工具，分析该指标的变化，是否有风险
流程5，可选流程。该指标的prometheus数据也单独发送AI 大模型分析下。
流程6，填表。
